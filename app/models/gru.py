"""
app/models/gru.py — PyTorch GRU (alternative sequential model)
app/models/transformer.py — PyTorch Transformer with temporal attention
app/models/arima.py — ARIMA statistical model
app/models/garch.py — GARCH volatility model
"""
...
class GRUModel(BaseMLModel):
    def __init__(self, version: str = "v1", hidden_size: int = 128, num_layers: int = 2,
                 lookback: int = LOOKBACK_SEQUENCE, epochs: int = 50, batch_size: int = 64, lr: float = 0.001) -> None:
        super().__init__(model_name="gru", version=version)
    ...
    def train(self, X: pd.DataFrame, y: pd.Series, **kwargs: Any) -> dict:
        logger.info("Training GRU", version=self.version, samples=len(X))
        ...
        if len(X_seq) == 0:
            return {}
        self._net = _GRUNet(X_seq.shape[2], self.hidden_size, self.num_layers).to(self._device)
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        if self._net is None:
            raise RuntimeError("GRU not trained")
    ...
    def save(self, path: Path) -> None:
        ...
        torch.save(self._net.state_dict() if self._net else {}, path / f"gru_{self.version}.pt")
        with open(path / f"gru_{self.version}_meta.pkl", "wb") as f:
            pickle.dump({"scaler": self._scaler, "features": self._feature_names,
                         "hidden_size": self.hidden_size, "num_layers": self.num_layers, "lookback": self.lookback}, f)

    def load(self, path: Path) -> None:
        with open(path / f"gru_{self.version}_meta.pkl", "rb") as f:
            meta = pickle.load(f)
        self._scaler = meta["scaler"]
        self._feature_names = meta["features"]
        self._net = _GRUNet(len(self._feature_names), meta["hidden_size"], meta["num_layers"]).to(self._device)
        state_path = path / f"gru_{self.version}.pt"
